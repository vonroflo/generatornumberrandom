...
export default function RandomNumberPickerTool() {
  ...
  const pick = () => {
    ...
    if (items.length === 0) {
      setError("Please enter some numbers to pick from");
      setResults([]);
      return;
    }

    if (quantity > items.length) {
      setError(`Can't pick ${quantity} items from a list of ${items.length}`);
      setResults([]);
      return;
    }
  ...
  return (
    <ToolCard>
      <div className="space-y-6">
        {/* Input Field */}
        <div>
          <label htmlFor="numbers" className="block text-sm font-medium mb-2 text-foreground-muted">
